...
class PurchaseStatistic:
    ...
    def execPurchaseHistory(self, tableName=None):
        sql = "select * from %s" % tableName if tableName else "select * from purchasehistory"
    ...
    def processAgeDistribution(self, fromAge=None, toAge=None):
        if fromAge is not None and toAge is not None:
    ...
    def processGenderAndCategoryCounter(self):
        self.df_gender_order = self.df.groupby(['gender', 'category']).size().reset_index(name="count")
    ...
    def visualizeBarPlot(self, df, columnX, columnY, hueColumn, title, alpha=0.8, width=0.6):
        ...
        sns.barplot(data=df, x=columnX, y=columnY, hue=hueColumn, alpha=alpha, width=width)
        plt.title(title)
        plt.xlabel(columnX)
        plt.ylabel(columnY)
        plt.grid()
    ...
    def processAgeOrderFrequence(self):
        self.dfAgeGender = self.df.groupby(['age', 'gender']).size().reset_index(name="count")
    ...
    def processMonthlySalesAmount(self):
        ...
        self.dfMonthlySalesAmount['invoice_date'] = pd.to_datetime(self.dfMonthlySalesAmount['invoice_date'], format='%d/%m/%Y')
    ...
    def visualizeLinePlotChart(self, df, columnX, columnY, title, hue=None):
        plt.figure(figsize=(8, 6))
        plt.ticklabel_format(useOffset=False, style="plain")
    ...
    def processMonthlyAndYearSalesAmount(self):
        ...
        self.dfMonthlyAndYearSalesAmount['invoice_date'] = pd.to_datetime(self.dfMonthlyAndYearSalesAmount['invoice_date'], format='%d/%m/%Y')
